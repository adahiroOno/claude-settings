...
1. `install.sh` が `home/` の内容を `~/.claude/` に導入します(既存ファイルは `~/.claude/backup-<日時>/` に退避)。テンプレート所有物(フック・statusline・スキル)は更新で上書きしますが、**あなたの内容である `settings.json` と `CLAUDE.md` は上書きしません**(下記)。
