...
- `settings.json` は**保持マージ**され、あなたが加えた設定(env・認証・独自 permissions/hooks)は保たれます。**マージ結果が既存と同一なら書き換えず**「変更なし・スキップ」と表示します(不要な再書き込み・再バックアップをしません)。差分があるときは、**どの項目が上書きされたかを `キー: 既存 → 適用後` の形で1件ずつ表示**します(例: `env.CLAUDE_SESSION_BUDGET_USD: "20" → "5"`)。あなたのコスト関連設定が黙って書き換わることはなく、意図して維持したい値はその項目を再編集すれば次回以降は維持されます(新規追加キーは上書きではないため件数で要約)。
